---
title: "B-MIS Normalization QE PelletsVesicles_ReExtractionJune2017_CyanoDCM_VarInjVol"
author: "Angie Boysen"
date: "Feb 9, 2017"
...
Location <- getwd()
Fraction <- "CyanoAq"
FractionShort <- "CyanoAq" #HILIC or CyanoAq or CyanoDCM
ExpDIR <-
...
IS.dat <- read_csv("Ingalls_Lab_CyanoAq_VesiclesPro_QE_Transition Results.csv")
IS.dat <- IS.dat %>%
...
IS.dat$Area[(IS.dat$MassFeature == "Heavy IAA") & 
                    (grepl("Pro|Std", IS.dat$`Replicate Name`))] <- errors$Area


 errors <- IS.dat %>%
      filter(MassFeature=="Heavy Tryptamine") %>%
      filter(grepl("Pro|Std", `Replicate Name`)) %>%
      mutate(Area2 = Area) %>%
      mutate(Area = Area/5)
...
IS.dat$Area[(IS.dat$MassFeature == "Heavy Tryptamine") & 
                    (grepl("Pro|Std", IS.dat$`Replicate Name`))] <- errors$Area
...
```{r, echo=FALSE}
q <- ggplot(IS.dat, aes(x=`Replicate Name`, y=Area)) + geom_bar(stat="identity") + facet_wrap( ~MassFeature, scales="free_y")+theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust = 0.5), legend.position = "top")  + ggtitle("IS Raw Areas")
```
...
xcms.long <- xcms.long %>%
  mutate(Replicate.Name = Replicate.Name %>%
           str_replace("Blk_1","Blk_1_")%>%
           str_replace("Blk_2","Blk_2_") %>%
...
#glimpse(area.norm)
write_csv(area.norm, "CyanoAq.area.norm.csv")
```
