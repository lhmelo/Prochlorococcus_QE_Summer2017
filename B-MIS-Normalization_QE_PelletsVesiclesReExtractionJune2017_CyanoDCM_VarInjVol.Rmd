...
#Downloaded from Ingalls Lab github 7/10/2017.  Requires Sample.Key.  Adjusts for variable injection volumes in pooled samples.
...
Location <- getwd()
Fraction <- "CyanoDCM"
FractionShort <- "CyanoDCM" #HILIC or CyanoAq or CyanoDCM
ExpDIR <-Location
...
IS.dat <- read_csv("Ingalls_Lab_CyanoDCM_VesiclesPro_QE_Transition Results.csv")
IS.dat <- IS.dat %>%
...
# print(xcms.names)
# 
allsamples <- xcms.names[grepl("_Smp|_Blk_2|_Blk_1|_ProcessBlk|_Poo|StdsInDCM", xcms.names)]
...
```{r correctISErrors}
 ...
 errors <- IS.dat %>%
      filter(MassFeature=="Heavy Archiodonic Acid") %>%
      filter(grepl("Pro|Std", `Replicate Name`)) %>%
      mutate(Area2 = Area) %>%
      mutate(Area = Area/10)

# IS.dat.test <- IS.dat %>% mutate(Area2 = Area)

IS.dat$Area[(IS.dat$MassFeature == "Heavy Archiodonic Acid") & 
...
#glimpse(area.norm)
write_csv(area.norm, "CyanoDCM.area.norm.csv")
```
